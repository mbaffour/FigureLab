...
- PDF export: two modes — JPEG-compressed raster (`/DCTDecode`) and **lossless** Flate (`/FlateDecode`, the default in the submission package). Panel labels and scale-bar text are laid down as real, selectable PDF text over the raster; the `/MediaBox` is the true physical page size. Multi-page PDF is the same writer with more than one page: the same supersampled canvas, the same page size, the same lossless option (on by default) and the same text layer on every page.
...
- **The shipped package describes the panels the figure actually draws.** The caption, the metadata CSV, the journal-compliance check and the package manifest all enumerated `images.slice(0, cols × rows)` — raw array order. A hidden panel still occupied one of those places, so the ZIP described panels the reader cannot see *and* silently dropped a visible one off the end: a ten-panel figure shipped a caption for twelve, a CSV whose rows ran 1–6 then 8–12, and a compliance line reading "11/11 panels labelled". All four now use the panels in reading order that the figure draws. `PANEL_HISTORY.txt` still records a hidden panel — you did process it — but marks it `[hidden from the figure]`.
- **Every plane of an OME-TIFF is calibrated, not just the first.** OME-TIFF and ImageJ write their description into page 0 only, and the calibration reader looked at the current page's. Frames 2…N of a real Bio-Formats or `tifffile` stack — which carry `PhysicalSizeX` and no resolution tags — therefore arrived **uncalibrated with their scale bar switched on and nothing to draw it from**. The description now falls back to page 0's, exactly as the acquisition-time reader already did. Per-plane `DateTime` is untouched and stays per plane.
- **In panel mode the channel key sticks to its panel.** 📌 *Panel mode* binds every annotation to the panel under the cursor — except the ▤ Key, which went on writing figure-level coordinates. Placing it looked identical; changing the H gap then left the key stranded on the background while its panel moved away. The Key tool's own tooltip promises it is "kept in step with the panels as they change".

- **New: rows hug their panels.** Every cell was the same height, so a wide short panel — a blot, a gel, a kymograph — floated in a tall cell with dead space above and below it, and closing the gaps did not help because the space was *inside* the cell. Stacking a blot over its loading control left a gulf between them that no setting could remove: **⤢ Fit panels to images** can only pick one shape for the whole grid, and says so. Tick **Rows hug their panels** (or ⌘K → *Rows hug their panels*) and each row shrinks to the tallest panel in it. Rows never grow past the panel height you set, panel width does not move, and the headings, group bands, gutter handles and every export follow the new heights.

- **Lane numbers, MW ladders and splice markers land on the blot.** All three store their position as a fraction of the whole figure. In a one-panel figure that is the same thing, which is why it went unnoticed — but in a 2 × 3 figure **↕ MW ladder** laid its weights down the left edge of the entire sheet, nowhere near the blot, and **⎸ Add splice marker** drew a line straight down the middle of the figure. They now measure from the selected panel (or the only one), place lane numbers above it and weights beside it when the margin has room, and name the panel they landed on.

- **Typed row and column headings stop disappearing.** The headings lived only in the text fields that showed them, and those fields are destroyed and rebuilt whenever the grid resizes or the "Column labels" box is ticked. Adding a column, hiding the headings, undoing an unrelated change, or saving a figure with the headings hidden all threw the text away — and the rebuilt field read `Col 3`, which looks like a heading you wrote and renders into the export as one. They now live in the figure's state, so they survive all of that and round-trip through saves, share links and undo.
- **The on-canvas heading toggle says what it does.** An 18 px chip in the top-left corner wore a `×`, had no tooltip and no hover state; one click on it hid every row and column heading. It now carries the same ⊞ mark as the sidebar's **⊞ Labels** button, lights up under the pointer, and reads "hide the row & column headings".
- **Greyscale, Invert and Reset are undoable.** All three are buttons, and the capture-before-edit handler that makes the sliders undoable only watches inputs and selects — so a click on **Reset**, which clears brightness, contrast, gamma, LUT and both level points at once, could not be taken back. Crop shape joins them.
- **One press of undo undoes a typed panel tilt.** The Tilt field recorded *two* undo steps — one before the edit from the generic handler, one after it from its own — so the first press restored the tilt you had just typed and appeared to do nothing.
- **Group bands are in the undo history.** They were left out of the state snapshot entirely, so the undo step that `setFigGroups` and the band ✕ button already recorded restored everything except the bands. Re-running a band generator also blanked the names you had typed; names now carry across when the band still covers the same rows.
- **Panels arrive in the order you picked the files.** Every import path is asynchronous, and they were all fired at once, so panels were committed in decode-completion order — a small PNG overtook a large TIFF, and a twelve-file drop landed shuffled. Panel letters are handed out at commit time, so `B` could end up sitting left of `A`. Files now open one at a time.
- **Relabelling with a panel hidden no longer makes two panels called B.** Hidden panels kept the letter they had while the visible run re-used it; they are now lettered after the visible ones, so the reader still sees A, B, C with no gaps and nothing is lettered twice.

- **Fixed (blocker): after straightening a panel you could never crop it.** A panel opens the crop editor with the whole frame selected but no box drawn; one press of 📐 Level, ✨ Auto, ↺ ↻ or the Tilt field counted that full frame as a box, so every later drag was treated as *moving* it — clamped to no movement — instead of drawing one. Straightening a specimen and then cropping it is the entire point of the tilt, and it could not be done.
- **Fixed (blocker): an annotation endpoint could not be dragged under its own toolbar.** The floating toolbar sits 54 px above the topmost endpoint and over the canvas, so dragging an endpoint upward moved it beneath the toolbar, which then swallowed the pointer and froze the drag — with no keyboard route to finish it, since grid annotations have no arrow-key nudge. The toolbar now steps out of the pointer's way for the duration of a drag.
- **Fixed: the crop editor showed the previous panel's tilt.** Opening it on a panel that had never been cropped displayed the last panel's angle, pixel box, and the amber "resampled — panel pixels are interpolated" warning, while the canvas simultaneously said "Draw a region to continue". An interpolation warning that isn't true is the wrong kind of wrong on a figure-integrity tool.
...
- **The ⚠ beside an uncalibrated panel is now a button.** Every panel ships with its scale bar switched on, so with no µm/px set the bar cannot be drawn and the only cue was a 5 px inert ⚠. It now reads **⚠ calibrate** and opens the calibration dialog.
- **Two misleading headings renamed.** "Label style" sits 335 px above the real panel-letter controls and styles row and column headers instead, so hunting for A/B/C found a block that does nothing — it is now "Row / column header style". "Scale Bar (global defaults)" could neither add a bar nor set its length; it is now "Scale bar appearance (all panels)" and its help says where length and calibration live.
- **Fixed: the "Rotate:" caption could wrap away from its own buttons**, leaving three unlabelled glyphs opening a row 955 px away at common laptop widths. The group is now one unit, with accessible names on the buttons.
- **The command palette now answers what you type.** It matched the whole query as one literal substring and then fell back to a letters-in-order match that hit almost anything — and rendered those guesses identically to real hits, so a miss looked like an answer. Worst case: typing **undo** and pressing Enter ran *Reset all crops*, because that entry's keywords contain the word. Now: every word of a query must land somewhere, a word-boundary hit outranks one buried inside a longer word, and a guess is shown under "Nothing matches … — did you mean:" instead of masquerading as a hit. 24 commands were added for things the palette simply could not reach — **Undo**, **Redo**, the panel gaps, panel size, export DPI, printed width, the arrow and shape tools, calibrating a scale bar, panel letters, merging and splitting channels, opening a saved session, and the journal check.
- **A palette entry can take you to a control, not just run one.** Where the answer is a setting rather than an action, the entry opens the section it lives in, scrolls to it, focuses it and flashes it — so "where do I set the gap between panels?" has an answer even though that field sits inside a section you would have to know to open.
- **Fixed: a per-gutter override silently outranked every uniform gap.** With *Per-gutter spacing* on, a gutter you had dragged kept its own value, and `Tighten spacing`, a journal preset and a loaded template all wrote the uniform H/V gap, reported success, and changed nothing for that gutter. Setting one gap for the whole figure now clears the per-gutter overrides and says how many went, so the figure always matches what the fields show.
- **Fixed: layout templates and share links dropped per-gutter spacing.** Neither saved `colGaps`/`rowGaps`/`advancedSpacing`, so a template lost it at save time and a recipient of a share link got a different figure with no warning. Loading a template also left the *previous* figure's overrides in place, which made the template's own gaps inert. Both now carry it, and loading a template replaces what was there rather than layering on it.
- **Fixed: a journal preset could not be undone.** It rewrites panel size, gaps, fonts and export width; with no undo entry, trying a house style to see what it looked like was a one-way door.
- **Fixed: page 2 of a multi-page PDF drew page 1's row gutters.** Row overrides were looked up by the row's position *on the page* rather than in the whole figure, so every page after the first silently mirrored page 1's spacing.
- **Fixed: a gutter drag that ended off the canvas never ended.** Release the mouse anywhere but the canvas — the surrounding page, browser chrome, another window — and the divider stayed "being dragged", so every later pointer move over the canvas rewrote the spacing with no button held. The gap ran away and only a click stopped it. A release anywhere now ends the drag, and a move with no button down ends one whose release we never saw. The same stranding affected the rotation grip, a label drag and a scale-bar resize; all four are fixed together.
- **Fixed: the − and + gutter chips were unclickable at any normal gap.** They are drawn about 25 px either side of the divider, but the divider's hover band is only `max(6, gap/2+6)` wide, so reaching for a chip left the band and wiped the chips before the click landed. Below a gap of roughly 41 px — including the default 6 and the 2 and 4 the journal presets set — both chips were permanently dead. They now stay alive while the pointer is on them.
- **Set the space between panels in the freeform editor.** There was no way to do it: *Distribute* spreads three or more objects between the outermost two, so with two selected it returned in **silence**, and at no count could you ask for an exact gap. The align bar gains **Space: [N] px** with ⇿ and ⇕, which puts exactly that many pixels between the selected objects on either axis, from two objects upward. Distribute now says why it cannot help instead of looking broken.
...
- **⊞ Fit grid to panels** (Arrange, and ⌘K): rows × columns for the visible panels with the fewest empty cells, landscape and compact — 6 → 3 × 2, 8 → 4 × 2, 5 → 3 × 2 rather than a strip.
- **⛶ Crop to fill cells** (Crop & Scale, and ⌘K): crops every panel to its cell's shape, symmetrically about the current crop, so the letterbox bars go. The converse of *Fit panels to images*, which reshapes the cells instead; both are one undo step.
- **Linked insets: draw the region, see it on the parent.** *Add linked inset…* opens the crop editor over what the parent actually shows, so the box you draw is the inset (lock the ratio to *this panel's cell* to fill it); the middle-third default stays in ⌘K. Each inset's region is outlined on its parent with the inset's letter, turning and flipping with the picture — the convention readers expect — with a toggle to hide the outlines. Drag an outline on the parent to move the inset's region; it keeps its size, stays inside the parent, and is one undo step. The generated caption now says what an inset is — "Magnified view of the boxed region in (A), 2.0× relative to (A)" — and that it is displayed with its parent's settings rather than repeating them.
- **Panel tags** (Look → Panel tags): a short text on each panel besides its letter — a condition, a time point — typed per panel, or generated as a **series** (0 min, 5 min, 10 min… with a pattern of `{v}`, `{u}`, `{n}`). With **Channel names** on, an empty tag names the panel's channels **each in its LUT colour** — DAPI in blue, GFP in green — the way QuickFigures and OMERO.figure do from metadata; here from the LUT, the extra channels and the filename. Vector text in SVG and PDF.
- **Intensity bar** per panel: a vertical LUT bar with the display range at its ends, in native units when the file's raw samples are kept, for pseudocoloured or grayscale panels. RGB photographs get none, since they have no single intensity axis.
- **Blank cells.** *Insert blank cell* (Arrange, and ⌘K) leaves a grid position empty — after the selected panel, or at the end — to hold a place or align a ragged row; it shows in the image list as a row you can remove. Empty cells had been possible by accident (a ragged multi-crop, a panel drag) and broke things: the caption and the session save threw on them, and every export drew the on-screen placeholder — a faint dashed box with the cell number — into the figure. All fixed; an empty cell is background in the export and keeps its place in a saved session.
- **Max and mean projections** (Images → Combine ticked panels, and ⌘K): tick panels of the same pixel size — the pages of a z-stack — and collapse them into one new panel, the brightest value at each pixel or the average, computed on the displayed 8-bit pixels and saying so in its caption note. Calibration and LUT come from the first ticked panel; the sources are kept.
- **Auto contrast** for panel letters, typed tags and scale bars: per panel, white or black from the brightness of the image beneath, so a white letter never lands on a white histology section and a white bar never vanishes into a bright-field image. Off by default; not applied under a label chip, which already guarantees contrast.
